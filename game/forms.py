...
class RegistrationForm(UserCreationForm):
    ...
    class Meta(UserCreationForm.Meta):  # Используем метаданные из стандартной формы UserCreationForm
        model = Player  # Указываем, что форма будет работать с моделью Player
        fields = ["username", "email", "password1", "password2"]  # Указываем поля, которые должны быть на форме (имя пользователя, email и пароли)

# Форма для редактирования профиля пользователя
class EditProfileForm(UserChangeForm):
    class Meta:
        model = Player  # Используем модель Player для редактирования данных пользователя
        fields = ["username", "email"]  # Поля, которые можно редактировать (имя пользователя и email)
...
class CharacterForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'name': forms.TextInput(attrs={'placeholder': 'Enter character name'}),  # Устанавливаем placeholder для поля имени персонажа
        }
